...
public:
    ...
    public:
        ...
        bool operator==(const RTParser::pair& rst) {
    ...
    RTParser(std::initializer_list<pair> pairs) : m_pairs(pairs) {
    }
    static RTParser parse(std::string str) {
        RTParser ret{};
        std::stringstream ss(str);
        std::string token{};
        while (std::getline(ss, token, '\n'))
    ...
    inline long long get_long(const std::string& key) {
